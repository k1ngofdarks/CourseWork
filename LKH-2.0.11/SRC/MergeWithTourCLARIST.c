...
void find_component_extent(rec * ptcur)
{
    ...
    stack = (rec **) malloc(Dimension * sizeof(rec *));
    ...
    while (top > 0) {
        ...
        if (ptcur->ptP->diftag == 0) {
            ptcur->ptP->diftag = difcnt;
            stack[top++] = ptcur->ptP;
        }
        if (ptcur->ptN->diftag == 0) {
            ptcur->ptN->diftag = difcnt;
            stack[top++] = ptcur->ptN;
        }
        if (ptcur->pt2P->diftag == 0) {
            ptcur->pt2P->diftag = difcnt;
            stack[top++] = ptcur->pt2P;
        }
        if (ptcur->pt2N->diftag == 0) {
            ptcur->pt2N->diftag = difcnt;
            stack[top++] = ptcur->pt2N;
        }
    }
    free(stack);
}
...
{
    rec *ptcom, *ptcur;
    rec **ptout = (rec **) malloc((MAXDIFNBR + 1) * sizeof(rec *));
    int *difcnt = (int *) malloc((MAXDIFNBR + 1) * sizeof(int));
